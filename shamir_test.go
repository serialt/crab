...
func TestSplit_invalid(t *testing.T) {
	...
	if _, err := ShamirSplit(secret, 1000, 3); err == nil {
		t.Fatalf("expect error")
	}

	if _, err := ShamirSplit(secret, 10, 1); err == nil {
		t.Fatalf("expect error")
	}
...
func TestSplit(t *testing.T) {
	...
	out, err := ShamirSplit(secret, 5, 3)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestCombine_invalid(t *testing.T) {
	...
	if _, err := ShamirCombine(parts); err == nil {
		t.Fatalf("should err")
	}
	...
	for i := 0; i < 5; i++ {
		for j := 0; j < 5; j++ {
			...
			for k := 0; k < 5; k++ {
				...
				recomb, err := ShamirCombine(parts)
				if err != nil {
					t.Fatalf("err: %v", err)
				}
